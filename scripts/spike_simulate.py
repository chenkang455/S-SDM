...
#? convert the imgs under sharp_folder to spike sequence on spike_folder
#? Structure as:
#?  base_folder
#?  ├── blur_folder
#?  ├── sharp_folder
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sharp_folder', type=str, default=r'E:\chenk\Data\GOPRO\test\raw_data')
    ...
    spike_folder = os.path.join(base_folder,'spike_data_csy')
    ...
    for dirpath, sub_dirs, sub_files in os.walk(sharp_folder):
        if len(sub_files) == 0 or sub_files[0].endswith(opt.img_type) == False:
            continue
        print(dirpath)
        output_folder = dirpath.replace(sharp_folder,spike_folder)
        ...
        spike_add = 20 # addtional spike out of the exposure period
